...
lowerAndGreater n list = show n ++ " is greater than " ++ show lower ++ " elements and lower than " ++ show greater ++ " elements"
                        where (lower, greater) = countRec (0,0) n list

countRec:: (Ord a) => (Int, Int) -> a -> [a] -> (Int, Int)
countRec (lower, greater) _ [] = (lower, greater)
countRec (lower, greater) n (x:xs) = countRec (if n>x then lower+1 else lower, if n<x then greater+1 else greater) n xs
